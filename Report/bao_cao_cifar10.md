...
| **B3 Fed 2 máy (Ethernet)** | 81.96% | 81.51% | **484.1s** | 9.08s | 43.9ms |

> `Train/round` và `Comm/round` là trung bình 2 client trên các round `ok` (bỏ round 1 cold start).
...
- **B3 (2 máy, 81.51%) ≈ B2 (1 máy, 81.59%)**: phân tán vật lý **không** làm giảm accuracy —
...
| Download model/round (Ethernet B3) | ~21.9ms |
| Communication/round (B3, ≈2×download) | **43.9ms** |
| Tỷ lệ comm / thời gian 1 round (~13.6s) | **~0.3%** |
...
Ngược với kỳ vọng "chia 2 máy sẽ nhanh hơn", **B3 (484s) chậm hơn B2 (361s) và B1 (250s)**.
Nguyên nhân **không phải communication** (chỉ 0.3%) mà là các chi phí đặc thù của hệ phân tán:

1. **Cold start một lần (Máy 2 join).** Round 1 của B3 mất **89.6s** (vs 30s ở B2) vì Máy 2 phải
   khởi động Python + PyTorch + tải CIFAR + init GPU rồi mới kết nối. Đây là độ trễ thiết lập,
   chỉ xảy ra 1 lần (nhờ `wait_timeout=120` để round 1 không bị bỏ lỡ).

2. **Mất cân bằng tải + rào đồng bộ (synchronous barrier).** FedAvg đồng bộ phải **chờ client
   chậm nhất** mỗi round. Trong B3:
   - client-0 (Máy 1): train **~10.1s/round** — vì Máy 1 **kiêm luôn server** (phục vụ gRPC,
     aggregation, evaluation) → tải nặng hơn.
   - client-1 (Máy 2): train **~8.1s/round** — máy chuyên dụng, chỉ chạy 1 client.
   → Round bị gate bởi node chậm (Máy 1 ~10.1s), nên tách 2 máy **không** rút ngắn được thời gian.

3. **Steady-state (bỏ round 1):** B3 = 394.4s vs B2 = 330.7s. Phần chênh chủ yếu do (2) — Máy 1
   làm việc kép khiến client-0 thành straggler.
...
| **Load imbalance** | Node kiêm server bị chậm hơn → cần cân nhắc tách server khỏi node train |
| **Startup latency** | Cold start Máy 2 (~60s) là chi phí một lần khi mở rộng cluster |
